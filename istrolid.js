...
// Generated by CoffeeScript 1.12.7
(function () {
	var blue, bluebrown, chooseNumber, chooseOne, darkness, fadered, genBox, genClouds, genDebree, genDodads, genRocks, genSymetrical, grayblue, greenbrown, greenpurple, lemondarkred, main, moonyellow, pinkpurple, randomVector, tanslate, tealwhite, whitepurple, yellowcyan, yellowpuce;
	...
	genSymetricalRTS = function () {
		...
		for (i = 0, ref = sim.playersPerTeam(); i < ref; i++) {
			...
			b = new types.SpawnPoint();
			b.side = sim.beta[i].side;
			b.spawn = sim.beta[i].number;
			b.pos[0] = -a.pos[0];
			b.pos[1] = a.pos[1];
			sim.things[b.id] = b;
		}
...
(function () {
	...
	window.Sim = (function () {
			...
			if (this.players == null) {
				...
				for (key in ref) {
					...
					if (p && p.connected) {
						...
						if (p.side == "alpha") this.alpha.push(p);
						if (p.side == "beta") this.beta.push(p);
					}
				}
			}
			this.generateMap();
		...
		Sim.prototype.generateMap = function (mapScale, numComPoints, mapSeed) {
			...
			this.numRocks = (Math.floor(Math.random() * (240 + 1))) * this.mapScale;
		...
		Sim.prototype.send = function () {
			...
			if (this.step % 16 === 0) {
				...
				for (z = 0, len8 = ref13.length; z < len8; z++) {
					player = ref13[z];
					if (config.allowedPlayers.includes(player.name) && player.connected) {
						send = true;
					}
				}
				if (send) {
	...
	window.Player = (function () {
		...
		Player.prototype.rqUnit = function (slot) {
			...
			if (sim.serverType === 'survival' && this.side === 'beta') {
				...
				spawn = sim.findSpawnPoint(this.side) || sim.findSpawnPoint(this.number);
				if (spawn) {
	...
	window.Trail = (function () {
		...
		Trail.prototype.grow = function (pos) {
			...
			if (this.trail.length === 0 || v2.distance(this.trail[this.trail.length - 1][0], pos) > 2) {
				return this.trail.push([
					[pos[0], pos[1]], intp.smoothStep
				]);
			}
		};
...
(function () {
	...
	parts.Mount360Micro = (function (superClass) {
		...
		Mount360Micro.prototype.rangeBuffMul = .75;
